...
public class MethodNamingConventionsRule extends AbstractJavaRule {
    ...
    public Object visit(ASTMethodDeclarator node, Object data) {
        ...
        if (Character.isUpperCase(methodName.charAt(0))) {
        	...
        	ViolationController.AddViolation(Violation.NAMING, data, node, "Method names should not start with capital letters");
        	
        }
        if (methodName.indexOf('_') >= 0) {
            //addViolationWithMessage(data, node, "Method names should not contain underscores");
            ViolationController.AddViolation(Violation.NAMING, data, node, "Method names should not contain underscores");        	
        }
        return data;
    }
