...
public class NameAnalysisNode extends AbstractASTAnalysisNode {
	...
	public RelationResult AnalysisAST(AbstractJavaNode analysisnode,ElementNode sourcenode) throws RelationAnalysisException {
		...
		if(MacroFunctions.NULLTrue(tmp)){
			return new RelationResult(name.getImage(),MethodAnlysistor.GetUnknownTypeName(),false);
		}
		...
		if (typenode1.getClass() == ASTVariableDeclarator.class)
		{
			...
			throw new RelationAnalysisException("ASTName에서" + typenode1.getClass().toString() + "타입을 못찾음...");
